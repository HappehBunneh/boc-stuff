...
def command():
    ...
    if query == 'DROP':
        ...
        results = [{str(k):float(str(v).replace('A', '').replace('V', '').replace('+', '').replace('C', '')) for k,v in i.items() if k in ['time', 'STACK_V', 'STACK_I', 'STACK_TEMP', 'OUTPUT_POWER'] and 'x' not in str(v)} for i in results]
        return str(results)
    else:
        return 'Dab'
# run the application
